...
colum_revive_lastSavedRuck=[[],[]]; //last saved ruck contents
colum_revive_lastRuckChanged=false; //ruck changed event
...
Colum_revive_Consulta_Vidas = {
	...
	if (_UserTryNumber > 0) then {diag_log[format["colum_revive: posible script lag deteted, try to log-in nº (%1) of player : %2, UID: %3, lifes: %4",_UserTryNumber,name _UserObj,_UserID,_tmpVidas]]};
	...
	if (!isnil 'ACE_fnc_receiverOnlyEvent' && _UserTryNumber <2) then 
	{
...
Colum_revive_fcn_Save_PJ= {
		private ["_UserObjt","_TmpPOs","_TmpArmas","_TmpStatus","_TmpMunicion","_UserVidas","_oldRuck","_UserRuck","_UserRevTime",
	"_TmpVehiculo","_TmpData","_TmpUncon","_TmpweaponBack","_TmpRuckData","_TmpOldData","_checkOldData","_saveData","_oldRevTime"];
	_UserObjt=_this select 0;
	_UserID=_this select 1;
	_UserRevTime=if((count _this)>2) then{_this select 2} else {-2};
	_UserRuck=if((count _this)>3) then{_this select 3} else {[]};
	...
	if (_saveData) then { //only save if first time or unit initialiced
		...
		diag_log[format["Colum_revive_Save: %1. %2",name _UserObjt,(if(count _this ==2) then {"(diconnect)"}else{"(playing)"})]];
	};
			...
			} foreach Colum_Revive_ID_list;
			{
				if (isplayer _x) then {
					if (!isnil 'ACE_fnc_receiverOnlyEvent') then 
					{
						...
						['colum_revive_FIJV', [_x,(getPlayerUID _x),-10]] call CBA_fnc_globalEvent;
					};
				};
				sleep 0.1;
...
Colum_revive_fnc_SaveClient= {
	private["_minimumSaveTime","_ruckMags","_ruckWeap","_ruckdata","_revtime"];
	_minimumSaveTime=7;
	...
	_revtime=if (Colum_revive_enLimbo || Colum_revive_extraTime) then {-10} else {((player getVariable "ace_w_revive") -time)};
	_savedata=[player,getplayerUID player,_revtime];
	if (Colum_revive_Save_Ruck) then {
...
Colum_revive_fnc_LimboStatus= {
	...
	private["_minimumSaveTime","_ruckMags","_ruckWeap","_ruckdata","_tmpRText"];
	...
	if (Colum_revive_Iniciado && ((Colum_revive_LifesPersist!= 0) || Colum_revive_DisconectSave)) then { //If the mission already started and 60 seconds have passed
		...
		while {(isNil "Colum_revive_VidasLocal") && (_W_clycle_count < 10)} do {
			if (!isNil 'ACE_fnc_clientToServerEvent') then
			{
				...
				['colum_revive_REQL', [getPlayerUID player,player,_W_clycle_count]] call CBA_fnc_globalEvent; // TODO: remove after stable 1.14 release
			};
