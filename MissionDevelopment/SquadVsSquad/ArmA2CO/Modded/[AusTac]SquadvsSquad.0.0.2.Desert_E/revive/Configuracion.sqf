...
Colum_revive_Conf_Lifes= paramsArray select 4;

//Disable IA wounding system for IA, seems to cause more desync.
if (paramsArray select 9 == 1) then {ace_sys_wounds_noai = true;}else{ace_sys_wounds_noai = false;};

//Total time that units remains uncon before die, in seconds
ace_wounds_prevtime = paramsArray select 5;
...
if (paramsArray select 8 == 1) then {ace_sys_wounds_withSpect = true;}else{ace_sys_wounds_withSpect = false;};
...
if (paramsArray select 10 == 1) then {Colum_revive_JIPTelep = true;}else{Colum_revive_JIPTelep = false;};
...
if (paramsArray select 6 == 1) then {Colum_revive_Respawn=true;} else {Colum_revive_Respawn=false;};
...
if (paramsArray select 6 == 1) then {Colum_revive_EndGameIfAllDie=[]} else {Colum_revive_EndGameIfAllDie=[west,east]}; //example value: [west,east]
...
//Time to appear the respawn buttons when you are wounded, in seconds
	ace_sys_spectator_RevShowButtonTime = (paramsArray select 9);
	...
	if (Colum_revive_Respawn_Side) then {
		...
		//Bluefor
		Colum_revive_RevButtons_WEST = ["Respawn"];
		Colum_revive_RespawnMarkers_WEST= ["respawn1_WEST"];
		Colum_revive_RespawnOffset_WEST=[0,0,0,0];
		
		//oppfor
		Colum_revive_RevButtons_EAST = ["Respawn"];
		Colum_revive_RespawnMarkers_EAST= ["respawn1_EAST"];
